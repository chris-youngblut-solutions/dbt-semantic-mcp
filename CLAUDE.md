# dbt-semantic-mcp

DuckDB + dbt medallion marts + a governed semantic layer (MetricFlow), exposed to agents over MCP

**Tier**: T1  **Language**: python  **Test discipline**: pragmatic

This file is **read by every Claude Code session** that opens this repo.
Keep it short, scannable, and load-bearing. Long-form prose belongs in
`docs/`. Personal scratch notes (TODOs, ad-hoc commands, half-formed
ideas) belong in `CLAUDE.local.md` (gitignored).

## What this is

A demo warehouse stack: synthetic seeds → dbt medallion models → MetricFlow
semantic layer → MCP server (stdio) answering metric queries from the governed
definitions. Two query paths answer the same governed questions: MetricFlow
(`mf` CLI) and an Ibis connector over a swappable SQL backend (DuckDB local by
default; Postgres / Snowflake / BigQuery by env). It does NOT compose SQL from
user input or run an orchestrator — see README Limits.

## Build / test / lint

All verbs go through `just`. Never run the underlying tool directly in
docs — that creates two sources of truth.

```sh
just            # list all recipes
just fmt        # auto-format
just lint       # lint (warnings → errors)
just test       # run tests per discipline (pragmatic)
just build      # build release artifact
just check      # fmt + lint + test (the merge gate)
```

Tier-2 release path:

```sh
just release patch    # tag + push (CI does the build/sign/SBOM/SLSA)
just release minor
just release major
```

## Architecture

- `warehouse/` — the dbt project (DuckDB; profiles.yml in-project). Run dbt/mf
  commands from this directory.
- `warehouse/seeds/*.csv` — generated by `scripts/generate_seed.py`, committed;
  a test enforces committed == regenerated. Never hand-edit a seed CSV.
- `src/dbt_semantic_mcp/` — MCP server.
  - `warehouse.py` — MetricFlow path; reads only dbt/MetricFlow artifacts
    (semantic_manifest.json, manifest.json, the `mf` CLI).
  - `connector.py` — Ibis path; builds the swappable backend
    (`DBT_SEMANTIC_MCP_BACKEND`, default duckdb) and derives aggregations from
    the governed semantic manifest. Creds are env/`.netrc`-sourced, never argv.
  - Keep both paths governed — no SQL composed from input.
- `tests/` — pragmatic discipline; the conftest builds the warehouse if
  target/ is missing.
- `.github/workflows/` — CI; SHA-pinned actions; runs dbt build +
  mf validate-configs before pytest.

## Conventions

- **Conventional Commits**, signed (`git commit -sS`).
- **Lockfile committed**.
- **Pre-commit installed** — `pre-commit install` once after clone.
- **GitHub Actions pinned by SHA** — Renovate / Dependabot keeps fresh.
- **Container base images pinned by digest** if any.
- **Structured logging** — no `print()` / `console.log` in committed code.

## License

See LICENSE files at repo root. Default scaffolding is Apache-2.0 OR MIT
dual (apache-mit-dual).

## See also

- `CLAUDE.local.md` (gitignored) — your scratch notes for this repo.
- `docs/adr/` — Architecture Decision Records.
- `~/.claude/CLAUDE.md` (user-scope) — global invariants that apply
  across every repo.
- `~/Documents/coding-standard.md` — the standard this repo conforms to.
